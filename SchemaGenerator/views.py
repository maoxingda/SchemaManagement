...
def schema_generate(request, dbconn_id):
    ...
    messages.info(request, mark_safe(f'<pre>{dbconn}: \n{locals_template}</pre>'))
    ...
    with psycopg2.connect(dw_dbconn.server_address()) as conn:
        with conn.cursor() as cursor:
            for table in dbconn.tables.all():
                ...
                max_column_lengh = max([len(column) for column, _, _, _, _ in columns])
                max_column_lengh = 25 if max_column_lengh < 25 else max_column_lengh

                ddl_sql = f'create table if not exists {req_schema}.{"db_" if req_schema == "ods" else ""}{dbconn.name}_{table.name}\n' \
                          f'(\n'
                ...
                if req_schema == 'emr':
                    ...
                    ddl_sql += f"location 's3_path'\n"
                ddl_sql += f";"

                if settings.DEBUG:
                    with open(f'ods.db_{dbconn.name}_{table.name}.sql', 'w') as f:
                        f.write('\n-- 建表语句\n' + ddl_sql)
